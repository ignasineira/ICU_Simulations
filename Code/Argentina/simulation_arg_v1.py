...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 15 15:09:52 2022
...
Nota. 
Ahora los tiempo es uci distribuyen nb
...
['total']
[ 7]#'T. CONFIRMACION PCR desde INICIO DE LOS SINTOMAS'
"""
import os
import sys
import warnings
...
from data_processing_arg import read_data,prepare_population
...
ID_list_day_new_variant=[366, 381, 395, 426, 454, 485];
p_shares_b117=[[0.0, 0.062, 0.115, 0.096, 0.0, 0.0]];
ID_list_day_b117=[366, 381, 395, 426, 454, 485];
window=29; 
probability_not_vac=[0.011];
probability_not_vac_b117=[0.0274];
probability_not_vac_new_variant=[0.024];
list_p_to_uci=[0.463];
list_n_to_uci= [8.8889];
list_p_to_uci_2021=[0.699];
list_n_to_uci_2021= [21.798];
ID_day_change_time=366;
...
def simulation_arg_v1(data,
    ...
    increase_vac1=np.array(increase_vac1)
    increase_vac2=np.array(increase_vac2)
    ...
    porc_uci1 = porc_uci_by_vac(porc_infected[0],D,G,T,W, 
                               prob_uci_vac1=prob_uci_vac1,
                               prob_uci_vac2=prob_uci_vac2)
    
    porc_uci1_new_variant = porc_uci_by_vac(porc_infected_new_variant[0],D,G,T,W, 
                               prob_uci_vac1=prob_uci_vac1_new_variant,
                               prob_uci_vac2=prob_uci_vac2_new_variant)
    
    porc_uci1_b117  = porc_uci_by_vac(porc_infected_b117[0],D,G,T,W, 
                               prob_uci_vac1=prob_uci_vac1_b117,
                               prob_uci_vac2=prob_uci_vac2_b117)
    
    
    porc_uci_final=porc_uci1+porc_uci1_new_variant+porc_uci1_b117
    ...
    """
    porc_uci_not_variant=porc_uci1
    porc_uci_new_variant=porc_uci1_new_variant
    porc_uci_b117=porc_uci1_b117
...
def porc_infected_by_vac(data,L,D,G,T,
    ...
    grilla_vac1=[
            np.concatenate([
                np.linspace(increase_vac1[j,i],increase_vac1[j,i],day_step[i],endpoint=False) for i in range(increase_vac1.shape[1]-1)],
                axis=None).tolist()[::-1] for j in range(increase_vac1.shape[0])]
    grilla_vac2=[
            np.concatenate([
                np.linspace(increase_vac2[j,i],increase_vac2[j,i],day_step[i],endpoint=False) for i in range(increase_vac2.shape[1]-1)],
                axis=None).tolist()[::-1] for j in range(increase_vac2.shape[0])]
    
    #dado que W=0 es el t-W se deven dar vuelta los valores
    grilla = np.array([[grilla_vac1,grilla_vac2]]) #(L,D,G,W)
...
def plot_uci_pred_sns(data, uci, W=29, pond=1, start_date='2020-07-01', end_date='2021-05-15', infected=False):
    ...
    for i, (group_name, group_df) in enumerate(df_res.groupby(["Grupo de edad"])):
        ...
        g = sns.relplot(x="start_date", y="uci",
                        hue="type",
                        #palette=palette,
                        kind="line", data=group_df, sizes=((15, 9)), legend=True)
